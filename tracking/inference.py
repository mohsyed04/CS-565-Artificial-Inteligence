    ...
    def observe(self, observation, gameState):
        ...
        A correct implementation will handle the following special case:
          *  When a ghost is captured by Pacman, all beliefs should be updated
             ...
             captured).
        """
        noisyDistance = observation #evidence
        emissionModel = busters.getObservationDistribution(noisyDistance)#emission=P(et|xt) where xt is current state, et is evidence
        ...
        for p in self.legalPositions:
            ...
            if emissionModel[trueDistance] > 0: # P(et|xt)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for p in self.legalPositions:
            ...
            for newPos, prob in newPosDist.items():
                allPossible[newPos] += prob * self.beliefs[p] # P(Xt+1|e1:t) = SUMxt P(Xt+1|xt) * P(xt|e1:t)
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
	listOfParticles=[]  #list of the particles
	numberOfParticles=0 #number of particles

	while numberOfParticles < self.numParticles:
	   for index in range(0,len(self.legalPositions)):
	      listOfParticles.append(self.legalPositions[index])
              numberOfParticles += 1	
    ...
    def observe(self, observation, gameState):
	   ...
	   for index in range(0,self.numParticles):
	      self.particles[index]=self.getJailPosition()
	else:
	   counter=util.Counter()
	   for index in range(0,self.numParticles): 
                counter[self.particles[index]] += 1 
	   for count in counter:	     
	      counter[count] = counter[count]*emissionModel[util.manhattanDistance(count, pacmanPosition)] #Getting beliefs
	   if counter.totalCount()==0: #Case 2
	      self.initializeUniformly(gameState)
	   else:
	      for index in range(0,self.numParticles):
	         self.particles[index]=util.sample(counter)
	...
	for index in range(0,len(self.particles)):
	   self.particles[index]=util.sample(self.getPositionDistribution(self.setGhostPosition(gameState, self.particles[index])))
	...
	counter.normalize()
        return counter #returns distribution
